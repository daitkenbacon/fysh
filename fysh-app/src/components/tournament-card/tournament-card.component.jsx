...
const TournamentCard = ({ tournament }) => {
    ...
    if (!currentUser) {
      ...
      try {
        ...
        updateDocInCollection("tournaments", id, {
          participants: [...participants, currentUserUID],
        });
  ...
  return (
    <div
      key={id}
      className={`group bg-gray-100 p-5 rounded-md relative duration-200 transform transition-opacity ${
    ...
    >
      ...
      <div className="hidden gap-4 flex-col group-hover:flex absolute font-bold top-1/4 left-1/2 -translate-x-1/2 text-center">
        <button className="bg-white text-xl p-1 rounded-md shadow hover:bg-gray-300 transition-colors hover:shadow-md">
          <RouterLink to={`/tournament/${id}`}>Details</RouterLink>
        </button>
        <button
          ...
          className={`${
            canRegister
              ? "bg-green-500 text-white hover:bg-green-600 hover:shadow-md transition-colors"
              : "bg-gray-500 text-white"
          } p-1 text-xl rounded-md shadow`}
        >
          {participants && participants.includes(currentUserUID)
            ? "Registered"
      ...
      <div className="mt-4 flex justify-between">
        <div>
          <h3 className="text-sm font-bold text-gray-700 mr-1">
            <RouterLink to={`/tournament/${id}`}>{name}</RouterLink>
          </h3>
        </div>
        <div
          className={`${
