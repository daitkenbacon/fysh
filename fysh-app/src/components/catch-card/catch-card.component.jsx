...
const CatchCard = (props) => {
  ...
  useEffect(() => {
    if (catches.length > 0) {
      ...
      setCatchItem(c);
      setCatchDate(
        new Date(c.time_submitted.seconds * 1000).toLocaleDateString("en-US")
      );
      setCatchTime(
        new Date(c.time_submitted.seconds * 1000).toLocaleTimeString("en-US")
      );
  ...
  return (
    <div
      className={`${
        isCatchOwner ? "bg-gray-500" : "bg-white"
      } max-w-sm rounded-lg overflow-hidden shadow-lg`}
    >
      <img
        onClick={() => openModal(catchItem)}
        src={catchItem.img}
        alt={catchItem.description}
        className="hover:opacity-70 cursor-pointer w-full rounded shadow max-w-sm h-48 object-cover"
      />
      <div className="px-6 py-3">
        <p
          className={`${
            isCatchOwner ? "text-white" : "text-gray-800"
          } text-base`}
        >
      ...
      <div className="flex flex-row">
        {isHost && !winner && (
          <button
            ...
            className="bg-blue-600 text-white hover:bg-blue-700 p-2 left-0 w-full bottom-0 declare-winner-button"
          >
            Declare winner
          </button>
        )}
        {isCatchOwner && isOpen && (
          <button
            ...
            className="bg-red-600 hover:bg-red-700 text-white p-2 right-0 bottom-0 w-full delete-catch-button"
          >
            Delete
          </button>
        )}
